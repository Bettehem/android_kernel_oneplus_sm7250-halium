...
module_param_call(sys_boot_complete, usb_enum_check, param_get_int,
			&sys_boot_complete, 0644);
...
/* @bsp, 2019/07/05 Battery & Charging porting */
	...
	pr_err("APSD_RESULT_STATUS = 0x%02x\n", stat);
...
int smblib_start_dpdm_recovery(struct smb_charger *chg, bool enable)
{
	int rc = 0;

	pr_err("enter >>>, enable:%d\n", enable);
	if (enable) {
		if (is_usb_present(chg)) {
			pr_err("Start recovering charger..!!");
			if (chg->dpdm_reg && chg->dpdm_enabled) {
				pr_err("locking dpdm mutex");
				mutex_lock(&chg->dpdm_lock);

				pr_err("Disable DPDM regulator");
				rc = regulator_disable(chg->dpdm_reg);
				if (rc < 0)
					pr_err("fail to disable dpdm reg rc=%d",
						rc);

				msleep(50);

				pr_err("Enable DPDM regulator");
				rc = regulator_enable(chg->dpdm_reg);
				if (rc < 0)
					pr_err("fail to enable dpdm reg rc=%d",
						rc);

				pr_err("unlocking dpdm mutex");
				mutex_unlock(&chg->dpdm_lock);
				pr_err("set USB switch to false before APSD rerun");
				set_usb_switch(chg, false);
				pr_err("Start APSD rerun");
				rc = smblib_masked_write(chg, CMD_APSD_REG,
					APSD_RERUN_BIT,
					APSD_RERUN_BIT);
				if (rc < 0)
		...
		pr_err("Stop recovering charger..!!");
	}
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	 ...
	 *      to handle RP_RP bit6 to detect as source
	 *      instead of DAM
	 */
...
#define PD_PANELOFF_CURRENT_UA		2000000
...
#define PD_FCC_CURRENT_UA 3800000
...
static void op_connecter_temp_check_work(struct work_struct *work)
{
	...
	if (!chg->count_run) {/*count run state keep count_total not change*/
		if (chg->connecter_temp >=
			chg->fast_check_threshold_temp) {// 33
			chg->count_total =
	...
	if (chg->connecter_temp >=
	    chg->first_protect_connecter_temp) { /* >=60 */
...
static void op_connecter_recovery_charge_work(struct work_struct *work)
{
	...
	pr_info("disconnect_vbus=%d, hw_detect=%d",
		chg->disconnect_vbus, chg->hw_detect);
	pr_info("connecter_temp=%d, interval_temp=%d",
		chg->connecter_temp, interval_temp);

	if ((chg->hw_detect == 0) &&
	    (chg->connecter_temp <= 50) &&
	    (interval_temp <= 10)) {
...
static void pdo_selection_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct smb_charger *chg = container_of(dwork,
				struct smb_charger, pdo_select_check_work);
	int retval = 0;
	static int pdo_select_retry_count;

	if (chg->pd_active) {
		retval = op_pdo_select(9000, 2000);
		if (retval == 0) {
			pr_err("%s(): pdo selection to 9V2A is successful\n",
				__func__);
			pdo_select_retry_count = 0;
		} else {
			pr_err("%s(): pdo selection to 9V2A fail:%d, ret=%d\n",
				__func__, retval, pdo_select_retry_count);
			// Schedule pdo selection after 5 sec and stop
			// if failed 3 times
			if (pdo_select_retry_count++ < 3)
				schedule_delayed_work(&chg->pdo_select_check_work,
					msecs_to_jiffies(PDO_SELECTION_INTERVAL_MS));
		}
		// Add check for third party PD+Fastcharger or PD+Superfast charger
		if (!chg->swarp_online &&
...
/* @bsp, 2019/12/20 add pd skin thermal check */
#define PD_CALLON_CURRENT_UA                 800000
#define PD_SKIN_THERMAL_NORMAL_CURRENT_UA    2000000
#define PD_SKIN_THERMAL_MEDIUM_CURRENT_UA    1500000
#define PD_SKIN_THERMAL_HIGH_CURRENT_UA      1000000
...
static void pd_skin_thermal_check_work(struct work_struct *work)
{
	...
	is_skin_temp_medium = check_skin_thermal_medium();
	if (is_call_on)
		ua = PD_CALLON_CURRENT_UA;
	else if (is_skin_temp_high)
		ua = PD_SKIN_THERMAL_HIGH_CURRENT_UA;
	else if (is_skin_temp_medium)
		ua = PD_SKIN_THERMAL_MEDIUM_CURRENT_UA;
	else
		ua = PD_SKIN_THERMAL_NORMAL_CURRENT_UA;

	icl = get_client_vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER);
	if (ua != icl) {
		pr_info("skin_thermal_temp=(%d), is_skin_thermal_medium(%d), is_skin_thermal_high(%d)\n",
...
static void switch_fast_chg(struct smb_charger *chg)
{
	...
	if (op_is_usb_switch_on(chg)) {
		...
		pr_info("dash alrady switched\n");
		return;
	}
	if (!is_usb_present(chg)) {
		mutex_unlock(&chg->sw_dash_lock);
		pr_info("dash usb is not present\n");
		return;
	}
...
static void op_chek_apsd_done_work(struct work_struct *work)
{
	...
	if (chg->usb_enum_status || chg->op_apsd_done || chg->pd_active) {
		pr_info("usb_enum_status or apsd_done or pd_active set, so return");
		chg->ck_apsd_count = 0;
		return;
	}
	rc = smblib_get_prop_usb_voltage_now(chg, &vbus_val);
	...
	struct smb_charger *chg, int ibat, int vfloat)
{
	...
	temp_region = op_battery_temp_region_get(chg);
	if (chg->pd_active && temp_region == BATT_TEMP_NORMAL) {
		vote(chg->fcc_votable,
			DEFAULT_VOTER, true, PD_FCC_CURRENT_UA);
	} else {
		vote(chg->fcc_votable,
...
static bool op_check_vbat_is_full_by_sw(struct smb_charger *chg)
{
	...
	if (tbatt_status == BATT_TEMP_LITTLE_COLD)
		vbatt_full_vol_sw =
			chg->vbatmax[tbatt_status] - chg->fv_offset_voltage_mv;
	...
	if (batt_volt > vbatt_full_vol_sw) {
		if (icharging < 0 && (icharging * -1) <= term_current) {
			...
			if (vbat_counts_sw > FULL_COUNTS_SW * 2) {
				vbat_counts_sw = 0;
				ret_sw = true;
				pr_info("Batt full by sw when charging>=0\n");
			}
		} else {
			vbat_counts_sw = 0;
			ret_sw = false;
...
static void handle_dash_skin_therm_temp(struct smb_charger *chg)
{
	...
	if (!lcd_status) { // LCD Off
		...
		if (!chg->is_skin_thermal_high) {
			if (chg->skin_thermal_temp >=
			    chg->skin_thermal_high_threshold_disp_on) { // >=40
				chg->is_skin_thermal_high = true;
				chg->is_skin_thermal_little_high = false;
			}
		} else {
			if (chg->skin_thermal_temp <
		    chg->skin_thermal_pre_high_threshold_disp_on) { // < 39
				chg->is_skin_thermal_little_high = true;
				chg->is_skin_thermal_high = false;
			}
		}

		// Skin therm little high
		if (!chg->is_skin_thermal_little_high) {
			if (!chg->is_skin_thermal_high &&
			chg->skin_thermal_temp >=
		chg->skin_thermal_little_high_threshold_disp_on) { // >= 38
				chg->is_skin_thermal_little_high = true;
				chg->is_skin_thermal_medium = false;
			}
		} else {
			if (chg->skin_thermal_temp <
		chg->skin_thermal_pre_little_high_threshold_disp_on) { // 37
				chg->is_skin_thermal_medium = true;
				chg->is_skin_thermal_little_high = false;
			}
		}
		...
		if (!chg->is_skin_thermal_medium) {
			if (!chg->is_skin_thermal_little_high &&
			    !chg->is_skin_thermal_high &&
			    chg->skin_thermal_temp >=
		    chg->skin_thermal_medium_threshold_disp_on) { // >=36
				chg->is_skin_thermal_medium = true;
			}
		} else {
			if (chg->skin_thermal_temp <
			    chg->skin_thermal_normal_threshold_disp_on) { // <35
				chg->is_skin_thermal_medium = false;
			}
		}
	}
...
static int op_get_skin_thermal_temp(struct smb_charger *chg)
{
	...
	if (chg->iio.op_skin_therm_chan) {
		...
		if ((thermal_temp >= 0) &&
		    (thermal_temp <= 60)) //filter to valid value
	...
	// Battery temperature in deci degrees
	batt_temp = get_prop_batt_temp(chg)/10;
	if (chg->pd_active) { // PD charging skin therm
		if (!chg->is_skin_thermal_high &&
			(chg->skin_thermal_temp >=
			chg->pd_skin_thermal_medium_threshold) && // 39
			batt_temp < 40) {
			chg->is_skin_thermal_medium = true;
			chg->is_skin_thermal_high = false;
		} else if (chg->is_skin_thermal_medium &&
			   (chg->skin_thermal_temp <
			    chg->pd_skin_thermal_normal_threshold) && // 38
			   batt_temp < 40)
			chg->is_skin_thermal_medium = false;

		if ((chg->skin_thermal_temp >=
		     chg->pd_skin_thermal_high_threshold) || //40
		    batt_temp >= 40) {
			chg->is_skin_thermal_high = true;
			chg->is_skin_thermal_medium = false;
		} else if (chg->is_skin_thermal_high &&
			   (chg->skin_thermal_temp <
			    chg->pd_skin_thermal_pre_high_threshold) && // 39
			   batt_temp < 40) {
			chg->is_skin_thermal_high = false;
			chg->is_skin_thermal_medium = true;
		}
	...
	pr_info("skin_thermal_temp=(%d),pd_active(%d)\n",
		chg->skin_thermal_temp,
		chg->pd_active);
	pr_info("SKIN THERM LEVEL: HIGH:%d, LITTLE_HIGH:%d MEDIUM:%d\n",
		chg->is_skin_thermal_high,
		chg->is_skin_thermal_little_high,
		chg->is_skin_thermal_medium);
...
bool check_skin_thermal_warm(void)
{
	...
	thermal_temp = op_get_skin_thermal_temp(g_chg);
	if (thermal_temp >= 0)
		return g_chg->is_skin_thermal_little_high;
	else
		return false;
}
...
static void smblib_panel_register_work(struct work_struct *work)
{
	...
	if (lcd_active_panel) {
		rc = drm_panel_notifier_register(lcd_active_panel,
			&chg->msm_drm_notifier);
		if (rc) {
...
/* @bsp, 2019/07/05 Battery & Charging porting */
	/* @bsp, 2019/10/22 Add for PD charging */
	INIT_DELAYED_WORK(&chg->panel_register_work,
			  smblib_panel_register_work);
	...
	INIT_DELAYED_WORK(&chg->op_check_high_vbat_chg_work,
			  op_check_high_vbat_chg_work);
	...
	INIT_DELAYED_WORK(&chg->pdo_select_check_work,
			pdo_selection_check_work);
	schedule_delayed_work(&chg->heartbeat_work,
			msecs_to_jiffies(HEARTBEAT_INTERVAL_MS));
	schedule_delayed_work(&chg->panel_register_work,
			      msecs_to_jiffies(2000));
	if (gpio_is_valid(chg->vbus_ctrl))
		schedule_delayed_work(&chg->connecter_check_work,
				msecs_to_jiffies(200));
...
// Temperature control center is not supported in 7250,
// Adding stub function to just return invalid value to align with fastcharge driver
int  __attribute__((weak))
op_get_cool_down_value(void)
{
	return -EINVAL;
}

void  __attribute__((weak)) op_pd_config_switch_normal(void)
{
}

#ifdef OP_SWARP_SUPPORTED
void  __attribute__((weak)) op_warp_config_for_swarp(void)
{
 // Stub functions to be compatible with common code
}
#endif
