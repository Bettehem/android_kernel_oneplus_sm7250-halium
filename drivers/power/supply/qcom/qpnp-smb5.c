...
/* USB ADC gpio channel names to read from DTSI */
#define USB_TEM1_ADC_CHANNEL_NAME	"usb_tem1_adc"
#define USB_TEM2_ADC_CHANNEL_NAME	"usb_tem2_adc"
...
/* @bsp, 20180713 Battery & Charging porting */
...
/* @bsp, 2019/06/28 vph sel set disable */
	...
	// Skin therm thresholds when display OFF
	OF_PROP_READ(node, "op,skin-thermal-high-threshold-disp-off",
			chg->skin_thermal_high_threshold_disp_off, retval, 1);
	if (chg->skin_thermal_high_threshold_disp_off <= 0)
		chg->skin_thermal_high_threshold_disp_off =
			SKIN_THERMAL_HIGH;
	pr_info("skin_thermal_high_threshold_disp_off=%d\n",
		chg->skin_thermal_high_threshold_disp_off);

	OF_PROP_READ(node, "op,skin-thermal-pre-high-threshold-disp-off",
		     chg->skin_thermal_pre_high_threshold_disp_off,
		     retval, 1);
	if (chg->skin_thermal_pre_high_threshold_disp_off <= 0)
		chg->skin_thermal_pre_high_threshold_disp_off =
			SKIN_THERMAL_PRE_HIGH;
	pr_info("skin_thermal_pre_high_threshold_disp_off=%d\n",
		chg->skin_thermal_pre_high_threshold_disp_off);

	OF_PROP_READ(node, "op,skin-thermal-medium-threshold-disp-off",
		     chg->skin_thermal_medium_threshold_disp_off,
		     retval, 1);
	if (chg->skin_thermal_medium_threshold_disp_off <= 0)
		chg->skin_thermal_medium_threshold_disp_off =
			SKIN_THERMAL_MEDIUM;
	pr_info("skin_thermal_medium_threshold_disp_off=%d\n",
		chg->skin_thermal_medium_threshold_disp_off);

	OF_PROP_READ(node, "op,skin-thermal-normal-threshold-disp-off",
		     chg->skin_thermal_normal_threshold_disp_off,
		     retval, 1);
	if (chg->skin_thermal_normal_threshold_disp_off <= 0)
		chg->skin_thermal_normal_threshold_disp_off =
			SKIN_THERMAL_NORMAL;
	pr_info("skin_thermal_normal_threshold_disp_off=%d\n",
		chg->skin_thermal_normal_threshold_disp_off);

	// Skin therm thresholds when display ON
	OF_PROP_READ(node, "op,skin-thermal-high-threshold-disp-on",
		     chg->skin_thermal_high_threshold_disp_on,
		     retval, 1);
	if (chg->skin_thermal_high_threshold_disp_on <= 0)
		chg->skin_thermal_high_threshold_disp_on =
			SKIN_THERMAL_HIGH;
	pr_info("skin_thermal_high_threshold_disp_on=%d\n",
		chg->skin_thermal_high_threshold_disp_on);

	OF_PROP_READ(node, "op,skin-thermal-pre-high-threshold-disp-on",
		     chg->skin_thermal_pre_high_threshold_disp_on,
		     retval, 1);
	if (chg->skin_thermal_pre_high_threshold_disp_on <= 0)
		chg->skin_thermal_pre_high_threshold_disp_on =
			SKIN_THERMAL_PRE_HIGH;
	pr_info("skin_thermal_pre_high_threshold_disp_on=%d\n",
		chg->skin_thermal_pre_high_threshold_disp_on);

	OF_PROP_READ(node, "op,skin-thermal-little-high-threshold-disp-on",
		     chg->skin_thermal_little_high_threshold_disp_on,
		     retval, 1);
	if (chg->skin_thermal_little_high_threshold_disp_on <= 0)
		chg->skin_thermal_little_high_threshold_disp_on =
			SKIN_THERMAL_LITTLE_HIGH;
	pr_info("skin_thermal_little_high_threshold_disp_on=%d\n",
		chg->skin_thermal_high_threshold_disp_on);

	OF_PROP_READ(node, "op,skin-thermal-pre-little-high-threshold-disp-on",
		     chg->skin_thermal_pre_little_high_threshold_disp_on,
		     retval, 1);
	if (chg->skin_thermal_pre_little_high_threshold_disp_on <= 0)
		chg->skin_thermal_pre_little_high_threshold_disp_on =
			SKIN_THERMAL_PRE_LITTLE_HIGH;
	pr_info("skin_thermal_pre_little_high_threshold_disp_on=%d\n",
		chg->skin_thermal_pre_little_high_threshold_disp_on);

	OF_PROP_READ(node, "op,skin-thermal-medium-threshold-disp-on",
		     chg->skin_thermal_medium_threshold_disp_on,
		     retval, 1);
	if (chg->skin_thermal_medium_threshold_disp_on <= 0)
		chg->skin_thermal_medium_threshold_disp_on =
			SKIN_THERMAL_MEDIUM;
	pr_info("skin_thermal_medium_threshold_disp_on=%d\n",
		chg->skin_thermal_medium_threshold_disp_on);

	OF_PROP_READ(node, "op,skin-thermal-normal-threshold-disp-on",
		     chg->skin_thermal_normal_threshold_disp_on,
		     retval, 1);
	if (chg->skin_thermal_normal_threshold_disp_on <= 0)
		chg->skin_thermal_normal_threshold_disp_on =
			SKIN_THERMAL_NORMAL;
	pr_info("skin_thermal_normal_threshold_disp_on=%d\n",
		chg->skin_thermal_normal_threshold_disp_on);
	...
	OF_PROP_READ(node, "op,pd-skin-thermal-medium-threshold",
			chg->pd_skin_thermal_medium_threshold, retval, 1);
	if (chg->pd_skin_thermal_medium_threshold <= 0)
		chg->pd_skin_thermal_medium_threshold =
			SKIN_THERMAL_MEDIUM;
	pr_info("pd_skin_thermal_medium_threshold=%d\n",
		chg->pd_skin_thermal_medium_threshold);

	OF_PROP_READ(node, "op,pd-skin-thermal-pre-high-threshold",
			chg->pd_skin_thermal_pre_high_threshold, retval, 1);
	if (chg->pd_skin_thermal_pre_high_threshold <= 0)
		chg->pd_skin_thermal_pre_high_threshold =
			SKIN_THERMAL_PRE_HIGH;
	pr_info("pd_skin_thermal_pre_high_threshold=%d\n",
		chg->pd_skin_thermal_pre_high_threshold);
	...
	if (rc < 0)
		chg->otg_cl_ua = (chip->chg.chg_param.smb_version ==
				PMI632_SUBTYPE) ? MICRO_1PA : MICRO_3PA;
...
/* @bsp 2019/07/05 add usb connector temp detect and wr*/
	rc = of_property_match_string(node, "io-channel-names",
			USB_TEM1_ADC_CHANNEL_NAME);
	...
	/* @bsp 2018/12/15 use the usbin_v_chan with low voltage charger */
...
/* @bsp, 2019/07/05 Battery & Charging porting */
	...
	case POWER_SUPPLY_PROP_CTM_CURRENT_MAX:
		...
		pr_info("POWER_SUPPLY_PROP_CTM_CURRENT_MAX:%d\n", val->intval);
		break;
	case POWER_SUPPLY_PROP_PR_SWAP:
...
/* @bsp, 2019/07/06 Battery & Charging porting */
	...
	case POWER_SUPPLY_PROP_FASTCHG_IS_OK:
		rc = 0;
		break;
	case POWER_SUPPLY_PROP_START_DPDM_RECOVERY:
		rc = smblib_start_dpdm_recovery(chg, val->intval);
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
...
static int create_skin_thermal_proc(void)
{
	...
	if (!proc_create("fastchg_thd", 0644, skin_threshold_dir, &proc_skin_thresh_disp_on_ops))
		pr_err("Failed to register fastchg_thd proc interface\n");

	if (!proc_create("fastchg_lcdoff_thd", 0644, skin_threshold_dir, &proc_skin_thresh_disp_off_ops))
...
/* @bsp, 2019/07/06 Fix abnormal animation */
...
/*usb connector hw auto detection*/
	...
	pr_info("QPNP SMB5 probed successfully yfb\n");
	return rc;
