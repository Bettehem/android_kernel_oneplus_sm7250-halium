...
#define SP_READ_LEVEL_INTERVAL_COUNT 3600
		...
		void *display, u32 bl_lvl)
{
	...
	old_backlight = panel->bl_config.bl_level;
	...
	if (strcmp(dsi_display->panel->name, "samsung ams644vk04 fhd cmd mode dsc dsi panel") == 0) {
	...
	if((old_backlight >= seed_low_backlight && bl_lvl < seed_low_backlight)
		|| (old_backlight < seed_low_backlight && bl_lvl >= seed_low_backlight)) {
		if (panel->naive_display_p3_mode) {
			if (bl_lvl >= seed_low_backlight) {
			...
			DSI_ERR("Send DSI_CMD_SET_NATIVE_DISPLAY_P3_ON cmds. the backlight is %d.\n", bl_lvl);
		}
		if (panel->naive_display_wide_color_mode) {
			if (bl_lvl >= seed_low_backlight) {
			...
			DSI_ERR("Send DSI_CMD_SET_NATIVE_DISPLAY_WIDE_ON cmds. the backlight is %d.\n", bl_lvl);
		}
		if (panel->naive_display_srgb_color_mode) {
			if (bl_lvl >= seed_low_backlight) {
			...
			DSI_ERR("Send DSI_CMD_SET_NATIVE_DISPLAY_SRGB_ON cmds. the backlight is %d.\n", bl_lvl);
		}
		if (panel->naive_display_customer_srgb_mode) {
			if (bl_lvl >= seed_low_backlight) {
			...
			DSI_ERR("Send DSI_CMD_LOADING_CUSTOMER_RGB_ON cmds. the backlight is %d.\n", bl_lvl);
		}
		if (panel->naive_display_customer_p3_mode) {
			if (bl_lvl >= seed_low_backlight) {
...
error:
	...
	if(((strcmp(dsi_display->panel->name, "samsung ams644vk04 ana6705 fhd cmd mode dsc dsi panel") == 0 ) ||
	    (strcmp(dsi_display->panel->name, "samsung ams644vk04 fhd cmd mode dsc dsi panel") == 0 )) && (0 == SERIAL_NUMBER_flag)) {
		dsi_display_get_serial_number_AT(connector);
	}
	if ((gamma_read_flag < 2) && ((strcmp(dsi_display->panel->name, "samsung ams644vk04 ana6705 fhd cmd mode dsc dsi panel") == 0) ||
	                              (strcmp(dsi_display->panel->name, "samsung ams644vk04 fhd cmd mode dsc dsi panel") == 0))) {
	...
	if ((sp_read_count < 2) && ((strcmp(dsi_display->panel->name, "samsung ams644vk04 fhd cmd mode dsc dsi panel") == 0)
		|| (strcmp(dsi_display->panel->name, "samsung ana6706 dsc cmd mode panel") == 0))) {
		if (sp_read_count < 1) {
			sp_read_count++;
		}
		else {
			dsi_display_sp_read(dsi_display);
			sp_read_count++;
		}
	}
	return rc;
...
int dsi_display_sp_read(struct dsi_display *dsi_display)
{
	...
	dsi_display_get_sp_level(dsi_display, panel);
...
int dsi_display_get_sp_level(struct dsi_display *dsi_display, struct dsi_panel *panel)
{
	int rc = 0;
	u32 flags = 0;
	u32 count = 0;
	...
	count = mode->priv_info->cmd_sets[DSI_CMD_READ_SAMSUNG_PANEL_REGISTER_SP_ON].count;
	if (!count) {
		DSI_ERR("This panel does not support sp register reading\n");
		goto exit;
	} else {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_READ_SAMSUNG_PANEL_REGISTER_SP_ON);
		if (rc) {
			DSI_ERR("Failed to send DSI_CMD_READ_SAMSUNG_PANEL_REGISTER_SP_ON commands\n");
			goto exit;
		}
	}

	cmds = mode->priv_info->cmd_sets[DSI_CMD_SET_PANEL_SP_LEVEL].cmds;
	...
	if (!m_ctrl->ctrl->vaddr)
		goto exit;
	cmds->msg.rx_buf = sp_register;
	cmds->msg.rx_len = 1;
	rc = dsi_ctrl_cmd_transfer(m_ctrl->ctrl, &cmds->msg, &flags);
	if (rc <= 0) {
		DSI_ERR("Failed to read DSI_CMD_SET_PANEL_SP_LEVEL rc=%d\n", rc);
		goto exit;
	}
	...
	if (sp_register[0] > 2) {
		count = mode->priv_info->cmd_sets[DSI_CMD_SET_SP_ON].count;
		if (!count) {
			DSI_ERR("This panel does not support sp on\n");
		} else {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_SP_ON);
			if (rc) {
				DSI_ERR("Failed to send DSI_CMD_SET_SP_ON commands\n");
				goto exit;
			}
			sp_read_flag = SP_READ_SUCCESS;
		}
		DSI_ERR("sp level is %02x, sp on\n", sp_register[0]);
	} else {
		count = mode->priv_info->cmd_sets[DSI_CMD_SET_SP_OFF].count;
		if (!count) {
			DSI_ERR("This panel does not support sp off\n");
		} else {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_SP_OFF);
			if (rc) {
				DSI_ERR("Failed to send DSI_CMD_SET_SP_OFF commands\n");
				goto exit;
			}
			sp_read_flag = SP_READ_ERROR;
		}
		DSI_ERR("sp level is %02x, sp off\n", sp_register[0]);
	}
...
release_panel_lock:
	...
	if (rc > 0) {
		if ((strcmp(dsi_display->panel->name, "samsung ams644vk04 fhd cmd mode dsc dsi panel") == 0)) {
			sp_read_wait_count++;
			if ((sp_read_wait_count == SP_READ_LEVEL_INTERVAL_COUNT)
				&& (sp_read_flag == SP_READ_ERROR)) {
				dsi_display_sp_read(dsi_display);
				DSI_ERR("dsi_display_sp_read\n");
				sp_read_wait_count = 0;
			}
		}
	}
	return rc;
...
static int dsi_display_res_init(struct dsi_display *display)
{
	...
	// TODO: add panel check if multiple panels
		...
		struct dsi_panel *panel, char *buf, int len)
{
	...
	if ((strcmp(panel->name, "samsung ams644vk04 ana6705 fhd cmd mode dsc dsi panel") == 0) ||
	    (strcmp(panel->name, "samsung ams644vk04 fhd cmd mode dsc dsi panel") == 0)) {
		...
		count = mode->priv_info->cmd_sets[DSI_CMD_SET_PANEL_IC_V].count;

		if (!count){
			DSI_ERR("This panel does not read SET_PANEL_IC_V register\n");
		} else {
	...
	if ((strcmp(panel->name, "samsung ams644vk04 ana6705 fhd cmd mode dsc dsi panel") == 0) ||
		...
		count = mode->priv_info->cmd_sets[DSI_CMD_SET_DDIC_CHECK_INFO].count;
		...
		panel->ddic_x = ddic_x;
		panel->ddic_y = ddic_y;
		DSI_ERR("LCD X = %d, Y = %d D6[3] = %d D6[4] = %d \n", panel->ddic_x, panel->ddic_y, register_d6[3] , register_d6[4]);
		flags = 0;
	}
...
int dsi_display_set_aod_mode(struct drm_connector *connector, int level)
{
	...
	if (strcmp(dsi_display->panel->name, "samsung ams644vk04 fhd cmd mode dsc dsi panel") == 0) {
		printk(KERN_ERR"OOPO dsi_display_set_aod_mode\n");
	} else if (strcmp(dsi_display->panel->name, "samsung ams644vk04 ana6705 fhd cmd mode dsc dsi panel") == 0) {
		printk(KERN_ERR"20801 dsi_display_set_aod_mode\n");
	} else {
...
//*vikas.kala add for support aod,hbm,seed*/
